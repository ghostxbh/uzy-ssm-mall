$(function () {
    ...
    $('#select_user_address_province').change(function () {
        $.ajax({
            type: "GET",
            url: "/bookstore/address/" + $(this).val(),
            data: null,
            dataType: "json",
...
function uploadImage(fileDom) {
    ...
    //上传图片
    $.ajax({
        url: "/bookstore/user/uploadUserHeadImage",
        type: "post",
        data: formData,
        contentType: false,
        processData: false,
        ...
        success: function (data) {
            if (data.success) {
                $(fileDom).prev("img").attr("src","/bookstore/res/images/item/userProfilePicture/"+data.fileName);
