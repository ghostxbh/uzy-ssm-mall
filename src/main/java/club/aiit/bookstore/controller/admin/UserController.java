package club.aiit.bookstore.controller.admin;

import club.aiit.bookstore.entity.*;
import club.aiit.bookstore.service.*;
...
public class UserController extends BaseController {
    ...
    //删除用户
    @ResponseBody
    @RequestMapping(value = "admin/user/delete/{arr}", method = RequestMethod.GET, produces = "application/json;charset=UTF-8")
    public String deleteUser(@PathVariable("arr") Integer[] user_id_list/* 用户id集合 */
    ) {
        logger.info("删除:用户id数组：" + user_id_list.toString());
        for (int i = 0; i < user_id_list.length; i++) {
            ...
            if (reviewList != null && reviewList.size() != 0) {
                ...
                if (reviewService.deleteList(review_id_list)) {
                    logger.info("已删除评论：" + review_id_list.length + "条");
                } else {
            ...
            if (productOrderItemList.size() != 0 && productOrderItemList != null) {
                ...
                if (productOrderItemService.deleteList(productOrderItem_id_list)) {
                    logger.info("已删除订单辅助表数据：" + productOrderItem_id_list.length + "条");
                } else {
                    throw new RuntimeException("删除订单辅助表数据异常");
                }

                if (productOrderService.deleteList(productOrder_id_list)) {
                    logger.info("已删除订单表数据：" + productOrder_id_list.length + "条");
                } else {
        ...
        if (userService.delete(user_id_list)) {
            ...
            logger.info("已删除用户：" + user_id_list.length + "条");
        } else {
